...
def leaderboard():
    url = f"https://statsapi.mlb.com/api/v1/stats/leaders?leaderCategories=hits,homeRuns,stolenBases,runsBattedIn,strikeouts,earnedRunAverage,battingAverage,saves,onBasePlusSlugging,wins&hydrate=person,team&limit=10&season=2023&sortColumn=hits&sortOrder=desc"
    response = requests.get(url)
    if response.status_code == 200:
        data = response.json()
